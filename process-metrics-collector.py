            ...
            with child.oneshot():
                ...
                if child.is_running() and child.status() != psutil.STATUS_ZOMBIE:
                    ...
                    if (
                        ...
                        # logging.error(f"Candidate docker {child_pid_int}")
                        # Matching
...
def process_metrics_collector(
    ...
    while p.is_running() and p.status() != psutil.STATUS_ZOMBIE:
        ...
        if len(docker_following_pids) > 0:
            ...
            for the_pid in docker_following_pids.keys():
                try:
                    ...
                    logging.error(f"ERROR: Process ID {the_pid} from docker not found.")
        ...
        for child_d, mode_w in children_dicts:
            ...
            csv_filename = dir_name / f"metrics-{child_pid_str}_{create_time}.csv"

            if mode_w == "w":
                logging.info(f"Writing data to CSV file {csv_filename.as_posix()}...")

                command_filename = (
                    dir_name / f"command-{child_pid_str}_{create_time}.txt"
                )
                command_json = dir_name / f"command-{child_pid_str}_{create_time}.json"
                ...
                if parent_create_time is not None:
                    ...
                    # logging.error(f"{child_pid} {parent_pid_str}")
                    parent_pid_str = "-"
